...
setup_proxy() {
    ...
    if command -v fwdproxy-config &>/dev/null; then
        ...
        if [[ -f "$cert" ]]; then
            git config --global http.proxy https://fwdproxy:8082
            git config --global http.proxySSLCert "$cert"
            git config --global http.proxySSLKey "$cert"
            export HTTPS_PROXY="https://fwdproxy:8082"
            CURL_PROXY="$(fwdproxy-config curl 2>/dev/null)"
            info "Forward proxy configured"
        else
...
set_default_shell() {
    ...
    if [[ "$SHELL" == "$zsh_path" ]]; then
        ...
        chsh -s "$zsh_path" 2>/dev/null && info "Default shell set to zsh" || warn "chsh failed — run manually: chsh -s $zsh_path"
    fi
}
